...
I've never really run into path-scoped cookies professionally, but I assume they're out there somewhere.
...
```d2 height="max(30%, 250px)" width="max(60%, 500px)"
vars: {
  d2-config: {
    theme-id: 101
    sketch: true
...
c1: "session_token\ndomain=app.example.com" {shape: page}


c2: "tracking_pixel\nexample.com\ndomain=<null>" {shape: page}
...
- [Next.js](https://github.com/vercel/edge-runtime/blob/main/packages/cookies/src/serialize.ts#L28)
- Laravel / Symfony / PHP stores cookies as an associative array, though for the life of me I can't find the source code.
...
This isn't really an issue for 99% of developers because it is _incredibly rare_ for some other code to
